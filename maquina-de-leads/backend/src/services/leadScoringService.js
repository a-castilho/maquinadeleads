...
const ADVANCED_STAGES = new Set(['contacted','responded','interested','converted','discarded']);

function normalizeText(value) {
  return String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
}
function tokenize(value) {
  return normalizeText(value).split(/[^a-z0-9]+/).map((item) => item.trim()).filter((item) => item.length >= 3);
}
...
function computeScore({ lead, campaign, keywords }) {
  ...
  if (name && normalizeText(name) !== 'desconhecido') add(10, 'Perfil identificado');

  if (lead.enrichment_status === 'enriquecido') add(15, 'Dados enriquecidos');
  else if (lead.descricao_extra) add(5, 'Descrição adicional disponível');
  ...
  const principalKeywords = keywords.filter((item) => item.kind === 'nicho').map((item) => item.term).filter(Boolean);
  if (includesAny(haystack, principalKeywords)) add(15, 'Aderência aos termos principais da campanha');

  const locationTokens = tokenize(campaign.location);
  if (locationTokens.length && locationTokens.some((token) => haystack.includes(token))) add(10, 'Indício de aderência à localização');
...
async function loadCampaign(nicheId) {
  const [campaignResult, keywordResult] = await Promise.all([
    db.query(`SELECT id, name, description, location, offer, target_audience, objective,
                    min_lead_score, campaign_status FROM niches WHERE id = $1`, [nicheId]),
    db.query(`SELECT term, kind FROM keywords WHERE niche_id = $1 AND active = true`, [nicheId]),
  ]);
  const campaign = campaignResult.rows[0];
  if (!campaign) throw new Error('Campanha não encontrada.');
  return { campaign, keywords: keywordResult.rows };
}
...
async function scoreBatch(nicheId, options = {}) {
  ...
  console.log(`[scoring] inicio niche=${nicheId} selected=${leads.length} threshold=${campaign.min_lead_score} force=${force}`);
  let qualified = 0;
  ...
  for (const lead of leads) {
    ...
    const nextStage = ADVANCED_STAGES.has(lead.funnel_stage) ? lead.funnel_stage : (isQualified ? 'qualified' : 'discovered');

    await db.query(
      `UPDATE leads SET lead_score = $1, score_breakdown = $2::jsonb, scored_at = NOW(),
                        funnel_stage = $3, updated_at = NOW()
        WHERE id = $4 AND niche_id = $5`,
      [calculated.score, JSON.stringify({ threshold: Number(campaign.min_lead_score || 55), reasons: calculated.breakdown }), nextStage, lead.id, nicheId]
    );
  ...
  console.log(`[scoring] fim ${JSON.stringify(result)}`);
  return result;
}

module.exports = { scoreBatch, computeScore, normalizeText };

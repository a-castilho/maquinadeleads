...
export default function GoogleMapsLeads() {
  ...
  async function search({ append = false } = {}) {
    if (!campaignId || !sector.trim() || !location.trim()) {
      setMessage('Selecione a campanha e informe setor e cidade/região.');
      return;
    }
    setSearching(true);
    setMessage('');
    try {
      ...
      setMessage(`${data.totalFiltered} leads compatíveis nesta página de ${data.totalRaw} resultados do Google Maps.`);
  ...
  async function importSelected() {
    ...
    setMessage('');
    try {
      const { data } = await api.post(`/niches/${campaignId}/google-maps/import`, {
    ...
    <div className="page google-maps-page">
      <header className="topbar google-maps-header">
        <div>
          ...
          <p className="hint">Busque empresas por setor e região, filtre por telefone, site, avaliação e importe apenas os leads úteis.</p>
        </div>
      </header>
      ...
      <section className="card maps-config-card">
        ...
        <form className="maps-key-form" onSubmit={saveGoogleKey}>
          ...
          <label><span>Google Places API key</span>
            <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder={configured ? 'Deixe vazio para manter a chave atual' : 'AIza...'} autoComplete="off" />
          </label>
      ...
      <section className="card maps-search-card">
        <div className="maps-card-heading"><div><h2>2. Busca e filtros</h2><p className="hint">Telefone vem primeiro. Você pode restringir ainda mais para WhatsApp móvel, site e reputação.</p></div></div>
        <div className="maps-search-grid">
          ...
          <label><span>Cidade / região</span><input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Ex.: Belo Horizonte MG" /></label>
          <label><span>Termo adicional</span><input value={extraTerm} onChange={(e) => setExtraTerm(e.target.value)} placeholder="Ex.: implantes, 24 horas, atacado" /></label>
        ...
        <button type="button" className="maps-search-button" onClick={() => search()} disabled={searching || !configured}>{searching ? 'Buscando no Google Maps...' : 'Buscar leads no Google Maps'}</button>
      ...
      <section className="card maps-results-card">
        <div className="maps-card-heading">
          <div><h2>3. Resultados</h2><p className="hint">{results.length ? `${results.length} leads carregados · ${selected.size} selecionados` : 'Faça uma busca para carregar empresas.'}</p></div>
